...
class DiceWeightedTrainer(SegmentationTrainer):
    def get_model(self, cfg=None, weights=None, verbose=True):
        model = DiceWeightedSegModel(cfg, nc=self.data["nc"], ch=self.data["channels"], verbose=verbose and RANK == -1)
        if weights:
...
def train_ablation_dice_weighted():
    args = dict(
        model=YAML,
        data=DATA_YAML,
        epochs=EPOCHS,
    ...
    trainer = DiceWeightedTrainer(overrides=args)
    results = trainer.train()
    return results


if __name__ == "__main__":
    train_ablation_dice_weighted()