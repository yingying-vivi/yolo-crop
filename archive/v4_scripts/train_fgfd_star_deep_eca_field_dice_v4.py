import torch
from ultralytics.nn.tasks import FieldDiceModel
...
class StarDeepECAFieldDiceTrainer(SegmentationTrainer):
    def get_model(self, cfg=None, weights=None, verbose=True):
        model = FieldDiceModel(cfg, nc=self.data["nc"], ch=self.data["channels"], verbose=verbose and RANK == -1)
        if weights:
...
def train_star_deep_eca_field_dice_v4():
    print(f"\n{'='*60}")
    print(f"[FGFD] Star-Deep + ECA + FieldDice Loss (freeze=0)")
    print(f"  Model: star-deep-eca (same structure)")
    print(f"  Mask loss: BCE + Dice (same as DiceSegLoss)")
    print(f"  Bbox loss: CIoU*0.8 + BoundaryIoU*0.2 (fixed, no decay)")
    print(f"  Loss dims: 5 (same as standard, no extra terms)")
    print(f"{'='*60}\n")
    ...
    args = dict(
        ...
        device=DEVICE,
        project=PROJECT,
        name="fgfd_star_deep_eca_field_dice_v4",
        exist_ok=True,
        workers=8,
        seed=42,
        freeze=0,
    ...
    trainer = StarDeepECAFieldDiceTrainer(overrides=args)
    results = trainer.train()
    return results


if __name__ == "__main__":
    train_star_deep_eca_field_dice_v4()