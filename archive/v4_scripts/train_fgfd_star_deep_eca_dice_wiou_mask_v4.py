...
class StarDeepECADiceWIoUMaskTrainer(SegmentationTrainer):
    def get_model(self, cfg=None, weights=None, verbose=True):
        model = FieldDiceWIoUModel(cfg, nc=self.data["nc"], ch=self.data["channels"], verbose=verbose and RANK == -1)
        if weights:
...
def train_star_deep_eca_dice_wiou_mask_v4():
    ...
    args = dict(
        ...
        device=DEVICE,
        project=PROJECT,
        name="fgfd_star_deep_eca_dice_wiou_mask_v4",
        exist_ok=True,
        workers=8,
        seed=42,
        freeze=0,
    ...
    trainer = StarDeepECADiceWIoUMaskTrainer(overrides=args)
    results = trainer.train()
    return results


if __name__ == "__main__":
    train_star_deep_eca_dice_wiou_mask_v4()
