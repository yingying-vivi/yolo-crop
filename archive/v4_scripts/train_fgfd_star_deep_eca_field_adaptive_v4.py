import torch
from ultralytics.nn.tasks import FieldAdaptiveModel
...
class StarDeepECAFieldAdaptiveTrainer(SegmentationTrainer):
    def get_model(self, cfg=None, weights=None, verbose=True):
        model = FieldAdaptiveModel(cfg, nc=self.data["nc"], ch=self.data["channels"], verbose=verbose and RANK == -1)
        if weights:
...
def load_with_shift(model, pretrained_weights, shift_map):
    if isinstance(pretrained_weights, str):
        ...
        pretrained_sd = pretrained_weights.float().state_dict()
...
def train_star_deep_eca_field_adaptive_v4():
    print(f"\n{'='*60}")
    print(f"[FGFD] Star-Deep + ECA + FieldAdaptive Loss (freeze=0)")
    print(f"  Mask loss: BCE(area-adaptive boundary weight) + Dice")
    print(f"  Bbox loss: standard CIoU (unchanged)")
    print(f"  Boundary pixel weight = 1 + boundary * (2/sqrt(area))")
    print(f"  Small parcels: boundary weight up to 6x")
    print(f"  Large parcels: boundary weight ~1x (almost no boost)")
    print(f"  Loss dims: 5 (standard, no extra dimensions)")
    print(f"{'='*60}\n")
    ...
    args = dict(
        ...
        device=DEVICE,
        project=PROJECT,
        name="fgfd_star_deep_eca_field_adaptive_v4",
        exist_ok=True,
        workers=8,
        seed=42,
        freeze=0,
    ...
    trainer = StarDeepECAFieldAdaptiveTrainer(overrides=args)
    results = trainer.train()
    return results


if __name__ == "__main__":
    train_star_deep_eca_field_adaptive_v4()