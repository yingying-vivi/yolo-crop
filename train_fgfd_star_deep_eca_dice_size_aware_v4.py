import torch
from ultralytics.nn.tasks import DiceSizeAwareModel
...
class DiceSizeAwareTrainer(SegmentationTrainer):
    def get_model(self, cfg=None, weights=None, verbose=True):
        model = DiceSizeAwareModel(cfg, nc=self.data["nc"], ch=self.data["channels"], verbose=verbose and RANK == -1)
        if weights:
...
def train_dice_size_aware_v4():
    print(f"\n{'='*60}")
    print(f"[FGFD] Star-Deep + ECA + Dice + Size-Aware Mask (CIoU bbox)")
    print(f"  Model: star-deep-eca")
    print(f"  Bbox loss: CIoU (standard, no WIoU)")
    print(f"  Mask loss: BCE/area*size_weight + Dice")
    print(f"  size_weight = sqrt(0.05/area), clamped [1.0, 4.0]")
    print(f"  Purpose: verify area-weighting contribution WITHOUT WIoU")
    print(f"{'='*60}\n")
    ...
    args = dict(
        ...
        device=DEVICE,
        project=PROJECT,
        name="fgfd_star_deep_eca_dice_size_aware_v4",
        exist_ok=True,
        workers=8,
        seed=42,
        freeze=0,
    ...
    trainer = DiceSizeAwareTrainer(overrides=args)
    results = trainer.train()
    return results


if __name__ == "__main__":
    train_dice_size_aware_v4()