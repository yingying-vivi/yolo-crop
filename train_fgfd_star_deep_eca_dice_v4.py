...
class StarDeepECADiceTrainer(SegmentationTrainer):
...
def train_star_deep_eca_dice_v4():
    print(f"\n{'='*60}")
    print(f"[FGFD] Star-Deep + ECA + Dice Loss (freeze=0)")
    print(f"  Model: star-deep-eca (same structure)")
    print(f"  Loss: v8SegLoss + Dice in seg_loss")
    print(f"  Only change: single_mask_loss = BCE + 0.5*Dice")
    print(f"  No extra loss terms, no dynamic decay, no boundary penalty")
    print(f"{'='*60}\n")
    ...
    args = dict(
        ...
        device=DEVICE,
        project=PROJECT,
        name="fgfd_star_deep_eca_dice_v4",
        exist_ok=True,
        workers=8,
        seed=42,
        freeze=0,
    ...
    trainer = StarDeepECADiceTrainer(overrides=args)
    results = trainer.train()
    return results


if __name__ == "__main__":
    train_star_deep_eca_dice_v4()